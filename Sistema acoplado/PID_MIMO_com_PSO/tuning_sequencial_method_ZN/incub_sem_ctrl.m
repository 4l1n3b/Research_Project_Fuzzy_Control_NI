...
plot(tempo/60, erro1, 'r', 'LineWidth', 1.5)
ylabel('Temperatura (°C)');
xlabel('Tempo(min)');
title('Saída de Temperatura - y_{1t}');
text((410*T_amost/60),28,{' \uparrow ', ' y_{1t}'});
text((410*T_amost/60),8,{'e_{1t}','\downarrow'});
grid on
hold off

subplot(2,2,4)
hold on
plot(tempo/60, y_2mfu,'b', 'LineWidth', 1.5)
plot(tempo/60, erro2, 'g', 'LineWidth', 1.5)
title('Saída de Umidade - y_{2u}')
text((120*T_amost/60),42,{'\uparrow ',' y_{2u}'});
text((120*T_amost/60),10,{'e_{2u}','\downarrow'});
ylabel('Umidade (%)');
xlabel('Tempo(min)');
grid on
hold off
