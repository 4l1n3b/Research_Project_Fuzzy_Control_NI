...
for k = 1:8
    ...
    tempo(k)=k*T_amost;
    erro1(k) = u_1t(k) - y_1mft(k);
    erro2(k) = u_2u(k) - y_2mfu(k);
end
...
% y_12mfu(k) = 0.9331 * y_12mfu(k-1) + a12_1* erro2(k-3) + a12_2 * erro2(k-4);
...
% y_22mfu(k) = 0.9048 * y_22mfu(k-1) + a22_1 * erro2(k-1) + a22_2 * erro2(k-2);
...
% plot(tempo/60, u_1t,'r--', 'LineWidth', 1)
% plot(tempo/60, u_2u, 'g', 'LineWidth', 1.5)
plot(tempo/60, y_1mft,'m', 'LineWidth', 1.5)
% plot(tempo/60, erro1,'b', 'LineWidth', 1)
% legend('Entrada 1 - u_{1}', 'Entrada 2 - u_{2}','Saída 1 - y_{1MF}');%, 'Erro 1 - e_{1MF}');
% title('Resposta da Saída 1 em Malha Fechada - y_{1MF}')
ylabel('y_{1}(°C)');
...
% plot(tempo/60, u_1t,'r--', 'LineWidth', 1)
% plot(tempo/60, u_2u, 'g:', 'LineWidth', 1.5)
plot(tempo/60, y_2mfu,'b', 'LineWidth', 1)
