...
error_zero_T  = triangular(u_disc_erro_temp, -7, 0, 7);
...
axis([-50 50 0 1.2]); grid on; hold off;
...
ylabel('Grau de Pertinência (\mu)');
axis([-5 5 0 1.25]); grid on; hold off;
...
error_zero_U  = triangular(u_disc_erro_umid, -12, 0, 12);
...
var_error_PN_U   = triangular(u_disc_Derro_umid, -7, -4.5, 0);
var_error_zero_U  = triangular(u_disc_Derro_umid, -3.5, 0, 3.5);
var_error_PP_U   = triangular(u_disc_Derro_umid, 0, 2, 7);
var_error_GP_U    = trapezoidal(u_disc_Derro_umid, 3.5, 5, 7, 8);
% Singletons de saida
vGPU = 1.1; vPPU = .3; vZU = 0; vPNU = -1; vGNU = -1.4;
...
%     switch i
%         case 1
%             filename = 'Tables/FuzzyTest/incubadora_var_param1Fuzzy.csv';
%         case 2
%             filename = 'Tables/FuzzyTest/incubadora_var_param2Fuzzy.csv';
%         otherwise
%             filename = 'Tables/FuzzyTest/incubadora_var_param100Fuzzy.csv';
%     end
