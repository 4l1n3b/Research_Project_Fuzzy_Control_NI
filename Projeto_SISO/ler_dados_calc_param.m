...
prompt = 'Qual a malha de controle? (1 - Temperatura, 2 - Umidade)\n';
malha = input(prompt);

while (malha ~= 1 && malha ~= 2)
...
if malha == 1
    ...
    elseif opcao_controle == 2
        ...
        pasta = 'fuzzy_valendo/fuzzy_sem_toolbox/Tables/';
        sufixo = 'Fuzzy_Temp.csv';
    end
elseif malha == 2
    if opcao_controle == 1
        pasta = 'Tables/';
...
for i = 1:length(cenarios)
    ...
    if exist(nome_arquivo, 'file')
        ...
        if malha ==1
            ...
            p2_t = calc_metrics(y_1temp(lim_p2+1:lim_p3), u_1temp(lim_p2+1:lim_p3), erro1temp(lim_p2+1:lim_p3), Tempo(lim_p2+1:lim_p3), 'Temperatura', 'P2 (36°C)', c);
            p3_t = calc_metrics(y_1temp(lim_p3+1:qde_amostras), u_1temp(lim_p3+1:qde_amostras), erro1temp(lim_p3+1:qde_amostras), Tempo(lim_p3+1:qde_amostras), 'Temperatura', 'P2 (33°C)', c);
         
            % Acumula no array de structs
            Todos_Resultados = [Todos_Resultados, p1_t, p2_t, p3_t];
        elseif malha == 2
             % Extrai colunas 
            ...
            % UMIDADE: P1, P2 e P3
            p1_u = calc_metrics(y_2umid(lim_p1:lim_p2), u_2umid(lim_p1:lim_p3), erro2umid(lim_p1:lim_p3), Tempo(lim_p1:lim_p3), 'Umidade', 'P1 (55%)', c);
            p2_u = calc_metrics(y_2umid(lim_p2+1:lim_p3), u_2umid(lim_p2+1:lim_p3), erro2umid(lim_p2+1:lim_p3), Tempo(lim_p2+1:lim_p3), 'Umidade', 'P2 (60%)', c);
            p3_u = calc_metrics(y_2umid(lim_p3+1:qde_amostras), u_2umid(lim_p3+1:qde_amostras), erro2umid(lim_p3+1:qde_amostras), Tempo(lim_p3+1:qde_amostras), 'Umidade', 'P2 (50%)', c);            
            % Acumula no array de structs
            Todos_Resultados = [Todos_Resultados,p1_u, p2_u, p3_u];
...
if opcao_controle == 1
    caminho_salvar = 'Tables/Performance/report_metricsZN.csv';
elseif opcao_controle == 2
    caminho_salvar = 'Tables/report_metricsASTROM.csv';
elseif opcao_controle == 3
    caminho_salvar = 'Tables/Performance/report_metricsFUZZY.csv';
end
